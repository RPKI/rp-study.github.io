...
if test ! -d ${DATADIR}/archive
then
    mkdir -p ${DATADIR}/archive || fatal "Unable to mkdir ${DATADIR}/archive"
fi
...
# set the soft links
cd ${DATADIR}
ln -sf archive/${DAT_FILE} pyasn.current.dat
ln -sf archive/${RIB_FILE} rib.current.bz2
