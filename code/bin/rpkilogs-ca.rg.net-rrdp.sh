...
LOG_PARSER=/net/data/rpki/bin/parse-ca.rg.net-rrdp.pl
...
# NOTE: ca.rg.net file spec: access.log-YYYYMMDD.HHMMSS.gz
#
# get list of files less than 31 days old AND more than 2 days old
SRCFILES=`find ${SRCDIR} -type f -mtime +3 -mtime -30 -name "access.log-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9].[0-9][0-9][0-9][0-9][0-9][0-9].gz"`
...
do
    ...
    then
        ...
        if [ $? -ne 0 ]
        then
             fatal "${UPDATE_CSV} ... -o ${_tmp_file} -u failed"
        fi
        ...
        export_csv_to_influx \
            ...
            -m connect \
            -tc pp,protocol,asn \
            -fc saddr,ua,file,ptr > ${_log_file}
        if [ $? -ne 0 ]
        then
